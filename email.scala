...
val emails=spark.sql("select partition.cluster_id,email_id,count(*),rank() over(partition by cluster_id order by cluster_id) as rank from partition left join cust on partition.source_system_id=cust.source_system_id group by cluster_id,email_id")
emails.show() 
